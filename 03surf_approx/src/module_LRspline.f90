module module_LRspline
  ...
  public ndeg, ncp, &
       ...
       set_open_uniform_global_knot, &
       set_Greville, &
       set_Bspline, &
       refine_LRspline, &
       draw_LRmesh, draw_controlpoints, draw_surface, &
       basis_func, diff_basis_func
...
contains
  ...
  !> 開一様 global knotを設定
  subroutine set_open_uniform_global_knot(ncp,tn1,tn2)
    integer,intent(in) :: ncp(0:ndim) !< 制御点の数
    real(8),intent(out) :: tn1(0:ncp(1)+ndeg(1)+1) !グローバル
    real(8),intent(out) :: tn2(0:ncp(2)+ndeg(2)+1) !ノット
    integer :: i
    ...
    do i=ndeg(1)+1,ncp(1)
       tn1(i)=dble(i-ndeg(1))/dble(ncp(1)-ndeg(1)+1)
    end do
    do i=ncp(1)+1,ncp(1)+ndeg(1)+1
       tn1(i)=1.d0
    end do
    ...
    do i=ndeg(2)+1,ncp(2)
       tn2(i)=dble(i-ndeg(2))/dble(ncp(2)-ndeg(2)+1)
    end do
    do i=ncp(2)+1,ncp(2)+ndeg(2)+1
       tn2(i)=1.d0
    end do

  end subroutine set_open_uniform_global_knot
  ...
  !> Bスプライン基底関数の微分 (local knot版) d/dt (B^np[tn(0),tn(1),...,tn(np+1)](t))を計算
  function diff_basis_func(np,tn,t) result(ret)
    integer,intent(in) :: np !< Bスプラインの次数
    real(8),intent(in) :: tn(0:np+1) !< local knot vec
    real(8),intent(in) :: t  !< argument of Bspline
    real(8) :: ret !< result (differentiated Bspline value)
    
    real(8) :: d1, d2

    if(tn(np)-tn(0).le.tiny) then
       d1=0.d0
    else
       d1=1.d0/(tn(np)-tn(0))
    end if
    if(tn(np+1)-tn(1).le.tiny) then
       d2=0.d0
    else
       d2=1.d0/(tn(np+1)-tn(1))
    end if
    ret=np*d1*basis_func(np-1,tn(0:np),t)-np*d2*basis_func(np-1,tn(1:np+1),t)
    
  end function diff_basis_func
